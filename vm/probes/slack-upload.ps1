...
# where a missing `files:write` scope, a placeholder SLACK_BOT_TOKEN asset, or an app that was never
...
Write-Output ('[probe] SLACK_BOT_TOKEN ' + $(if ($env:SLACK_BOT_TOKEN) { "present, $($env:SLACK_BOT_TOKEN.Length) chars" } else { 'MISSING - the asset password is empty' }))
...
Write-Status @{ posted = [bool]$posted; tokenPresent = [bool]$env:SLACK_BOT_TOKEN; thread = $THREAD_TS }
